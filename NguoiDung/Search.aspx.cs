...
public partial class NguoiDung_Search : System.Web.UI.Page
{
    ...
    private DataTable GetDataTable()
    {
        ...
        dtItems = (DataTable)Session["Search"];
        return dtItems;
    }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            SanPhamDTO spDTO = new SanPhamDTO();
            if (txtTen.Text != "")
                spDTO.Ten = txtTen.Text;
            else
                spDTO.Ten = null;
            if (txtTuGia.Text != "")
                spDTO.GiaNho = int.Parse(txtTuGia.Text);
            else
                spDTO.GiaNho = 0;
            if (txtDenGia.Text != "")
                spDTO.GiaLon = int.Parse(txtDenGia.Text);
            else
                spDTO.GiaLon = 0;
            spDTO.NSX = ddlNSX.SelectedItem.Value.ToString();
            spDTO.LoaiSP = int.Parse(ddlLoai.SelectedItem.Value.ToString());
            DataTable dt = SanPhamBUS.Search(spDTO);
            if (dt.Rows.Count > 0)
            {
                Panel2.Visible = true;
            ...
            {
                Response.Write("<script type='text/javascript'>alert('Không tìm thấy sản phẩm yêu cầu');</script>");
